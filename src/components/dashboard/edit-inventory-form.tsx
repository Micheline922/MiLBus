...
type EditInventoryFormProps = {
  ...
  onSubmit: (values: { products: Product[], wigs: Wig[], pastries: Pastry[] }) => void;
};

export default function EditInventoryForm({ initialValues, onSubmit }: EditInventoryFormProps) {
          ...
          <div className="space-y-4">
            ...
            {wigFields.map((field, index) => (
                <div key={field.id} className="relative grid grid-cols-2 gap-4 p-4 border rounded-lg">
                    ...
                    <FormField control={form.control} name={`wigs.${index}.wigDetails`} render={({ field }) => ( <FormItem><FormLabel>Détails Perruque</FormLabel><FormControl><Input {...field} /></FormControl><FormMessage /></FormItem> )} />
            ...
            <Button type="button" variant="outline" size="sm" onClick={() => appendWig({ id: `w${wigFields.length + 1}`, purchasedBundles: '', brand: '', colors: '', wigDetails: '', bundlesPrice: 0, sellingPrice: 0, sold: 0, remaining: 0 })}>Ajouter une perruque</Button>
            ...
            <Button type="button" variant="outline" size="sm" onClick={() => appendPastry({ id: `pa${pastryFields.length + 1}`, name: 'Beignets', quantity: 0, unitPrice: 0, totalPrice: 0, expenses: 0, sold: 0, remaining: 0 })}>Ajouter une pâtisserie</Button>
